	...
	if(argc>1)
	{
		...
		while(getline(fin,line))
		{
			...
			else if(line.substr(0,3)=="pop")
			{
				...
				{
					fout << "@" << argv[1]<< "." << index << endl << "D=A" << endl << "@R13" << endl << "M=D" << endl 
					<< "@SP" << endl << "M=M-1" << endl << "A=M" << endl << "D=M" << endl << "@R13" <<
			...
			else if(line.substr(0,4)=="goto")
			{
				...
				int fspace = line.find(" ",0);
				int sspace = line.find(" ",fspace+1);
				string label = line.substr(fspace+1, sspace-fspace-1);
				fout<<label<<endl;
				fout<<"0;JMP"<<endl;
			}
			...
			else if(line.substr(0,4) == "call")
			{
				...
				int m = stoi(margs);
				fout << "@" <<filename<<"-RETURN" << fcount << endl << "D=A" << endl << "@SP" << endl << "A=M" << endl << "M=D" << endl;
				fout << "@SP" << endl << "AM=M+1" << endl;
				...
				fout << "("<<filename<<"-RETURN" << fcount << ")" << endl;
				fcount++;
			}
